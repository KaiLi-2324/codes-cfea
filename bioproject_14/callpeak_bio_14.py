...
def get_input_files(path_meta):
    ...
    cancer_sample_input = defaultdict(dict)
    raw_input = dict()
    with open(path_meta) as f:
        for line in f:
            ...
            if not re.search(r"\s+-input", line_list[8]):
                this_sample_cancer = line_list[8].strip()
                cancer_sample_input[this_sample_cancer]["raw"] = re.sub(r"\.sra$", "", os.path.basename(line_list[9]))
            else:
                this_sample_cancer = re.sub(r"\s+-input", "", line_list[8].strip())
                cancer_sample_input[this_sample_cancer]["input"] = re.sub(r"\.sra$", "", os.path.basename(line_list[9]))
    for each_cancer in cancer_sample_input:
        if "input" in cancer_sample_input[each_cancer]:
            raw_input[cancer_sample_input[each_cancer]["raw"]] = cancer_sample_input[each_cancer]["input"]
        else:
            raw_input[cancer_sample_input[each_cancer]["raw"]] = "NA"
    return raw_input
...
def main():
    ...
    parser.add_argument("-r", action="store", dest="path_meta", help="path to the meta information[bioproject_14.meta_input.xls]")
