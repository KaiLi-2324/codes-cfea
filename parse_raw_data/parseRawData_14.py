...
def parse_file(path):
    ...
    with open(path) as f:
        for line in f:
            ...
            elif line.startswith("!Sample_characteristics_ch1 = sample name:"):
                line_list = line.strip().split(": ")
                sample_items[sample_name]["sample_name"].append(line_list[1])
            elif line.startswith("!Sample_characteristics_ch1 = tissue:"):
                line_list = line.strip().split(": ")
                sample_items[sample_name]["tissue"].append(line_list[1])
            elif line.startswith("!Sample_characteristics_ch1 = target molecule:"):
                line_list = line.strip().split(": ")
                sample_items[sample_name]["target_molecule"].append(line_list[1])
...
def main():
    if len(sys.argv) < 3:
        print(f"Usage: python parseRawData.py GSE81314_family.soft bioproject.14.meta.xls")
        exit(0)
    path = sys.argv[1]
    path_out = sys.argv[2]
