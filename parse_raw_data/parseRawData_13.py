...
def parse_file(path):
    ...
    with open(path) as f:
        for line in f:
            ...
            elif line.startswith("!Sample_characteristics_ch1 = disease site:"):
                line_list = line.strip().split(": ")
                sample_items[sample_name]["disease_site"].append(line_list[1])
...
def main():
    if len(sys.argv) < 3:
        print(f"Usage: python parseRawData.py GSE89570_family.soft bioproject.13.meta.xls")
        exit(0)
    path = sys.argv[1]
    path_out = sys.argv[2]
