...
def parse_file(path):
    ...
    with open(path) as f:
        for line in f:
            ...
            elif line.startswith("!Sample_characteristics_ch1 = cell line:"):
                line_list = line.strip().split(": ")
                sample_items[sample_name]["cell_line"] = line_list[1]
            elif line.startswith("!Sample_characteristics_ch1 = cell type:"):
                line_list = line.strip().split(": ")
                sample_items[sample_name]["cell_type"] = line_list[1]
            elif line.startswith("!Sample_label_ch1 ="):
                line_list = line.strip().split("= ")
                sample_items[sample_name]["sample_label"] = line_list[1]
            elif line.startswith("!Sample_treatment_protocol_ch1 = "):
                line_list = line.strip().split("= ")
                sample_items[sample_name]["treatment_protocol"] = line_list[1]
            elif line.startswith("!Sample_label_protocol_ch1 = "):
                line_list = line.strip().split("= ")
                sample_items[sample_name]["label_protocol"] = line_list[1]
