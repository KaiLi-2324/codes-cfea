...
def crawl_page(url):
    # for example, url is http://bigd.big.ac.cn/bioproject/browse/PRJCA000646
...
def write(path_out, each_sample_info):
    items = ["sample_name", "sample_title", "organism",
             "sample_description", "sample_tissue", "cell_type", "disease_state", "CRR"]
...
def main():
    if len(sys.argv) != 3:
        print("Usage: python parseRawData_new_2.py http://bigd.big.ac.cn/bioproject/browse/PRJCA000646 new.2.meta.xls")
        exit(-1)
    url = sys.argv[1]
    path_out = sys.argv[2]
