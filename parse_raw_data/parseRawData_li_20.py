    ...
    with open(path) as f:
        for line in f:
            if line.startswith("^SAMPLE = "):
                ...
                line_list = line.strip().split("= ")
                sample_items[sample_name]["molecule"].append(line_list[1])
            elif line.startswith("!Sample_data_processing = Genome_build:"):
                line_list = line.strip().split(": ")
                sample_items[sample_name]["genome_build"].append(line_list[1])
            elif line.startswith("!Sample_library_selection ="):
                line_list = line.strip().split("= ")
                sample_items[sample_name]["library_selection"].append(line_list[1])
...
def main():
    if len(sys.argv) < 3:
        print(f"Usage: python parseRawData.py GSE79277_family.soft liquid.20.meta.xls")
        exit(0)
    path = sys.argv[1]
    path_out = sys.argv[2]
